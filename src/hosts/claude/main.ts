...
//
// EXIT CODE DISCIPLINE. Claude Code reads a non-zero exit as a hook failure and
// exit 2 specifically as "block this call", so a crash in the counting path
// would refuse the user's tool call. Everything here funnels to exit 0 and an
// empty stdout; the only refusal this adapter ever issues is a deliberate
// `permissionDecision: "deny"` in opt-in block mode.
...
/** The whole adapter as one pure-ish string->string step, so a test can drive
 * a full hook invocation without a process. Returns "" when there is nothing
 * to say, which the runner turns into empty stdout. */
...
// Run directly when invoked as a script, so `node dist/hosts/claude/main.js` is
// a working hook command today. The unified `token-norm hook <host> <event>`
// binary (build order step 7) imports `main` instead of duplicating this.
